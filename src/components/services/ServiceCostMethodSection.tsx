...
export default function ServiceCostMethodSection({ categoryId, baseCost, salePrice, costMethod, indirectPct, indirectAmount, onCostUpdate, onSaved }: Props) {
  const { businessId } = useResolvedBusinessId();
  const [method, setMethod] = useState<CostMethod>((costMethod as CostMethod) || 'direct');
  const [pct, setPct] = useState(indirectPct ?? 0);
  const [dateFrom, setDateFrom] = useState('');
  const [dateTo, setDateTo] = useState('');
  const [realAmount, setRealAmount] = useState(indirectAmount ?? 0);
  ...
  useEffect(() => {
    ...
    const fetchExpenses = async () => {
      ...
      try {
        ...
        // For services, share is baseCost / totalServiceCosts * totalExpenses (simplified)
        if (totalExpenses > 0 && baseCost > 0) {
          setRealAmount(Math.round((totalExpenses * 0.1) * 100) / 100); // 10% share for services approx
        } else {
          setRealAmount(0);
        }
      } catch (err) {
  ...
  const margin = salePrice > 0 ? ((salePrice - adjustedCost) / salePrice * 100) : 0;

  const isDirty =
    method !== ((costMethod as CostMethod) || 'direct') ||
    (method === 'indirect_pct' && pct !== (indirectPct ?? 0)) ||
    (method === 'real_expenses' && realAmount !== (indirectAmount ?? 0));
        ...
        {METHOD_OPTIONS.map((opt) => (
          <button
            ...
            className={cn(
              ...
              method === opt.value ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted/50'
            )}
          >
      ...
      {method === 'indirect_pct' && (
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            <Input type="number" min={0} max={100} step={0.1} value={pct} onChange={(e) => setPct(Number(e.target.value))} className="w-28" />
