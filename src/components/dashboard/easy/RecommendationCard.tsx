...
const RecommendationCard = ({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[var(--te-r-lg)] bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-center gap-5 px-6 py-[22px] ${className || ''}`}
    >
      <div className="absolute inset-0 pointer-events-none" style={{ background: bgGradient }} />
      <div className={`relative w-11 h-11 flex items-center justify-center rounded-[var(--te-r-md)] flex-shrink-0 ${iconWrapCls}`}>
        {icon || defaultIcon}
      </div>
      <div className="flex-1 relative min-w-0">
        <div className={`text-[11px] font-semibold tracking-[0.6px] uppercase mb-1 flex items-center gap-2 ${labelCls}`}>
          <span>{label}</span>
          <span className="w-[3px] h-[3px] rounded-full bg-[var(--te-text-quaternary)]" />
          <span className="te-font-serif italic text-[12px] text-[var(--te-text-tertiary)] normal-case font-normal" style={{ letterSpacing: 0 }}>
            {labelSecondary}
          </span>
        </div>
        <div className="text-[16px] font-semibold text-[var(--te-text-primary)] mb-1">
          {title}
        </div>
        <div className="text-[13px] text-[var(--te-text-secondary)] leading-[1.5]">
          {description}
        </div>
      </div>
      <button
        onClick={onCta}
        className={`relative inline-flex items-center gap-1.5 px-3.5 py-2 rounded-[var(--te-r-md)] text-[13px] font-medium cursor-pointer flex-shrink-0 transition-opacity border-0 ${ctaCls}`}
      >
        {ctaLabel}
