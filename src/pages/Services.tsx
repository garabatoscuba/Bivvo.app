...
import { Plus, Pencil, Trash2, Loader2, Gamepad2, Monitor, Smartphone, Globe, DollarSign } from 'lucide-react';
...
const Services = () => {
  ...
  const saveCatMutation = useMutation({
    mutationFn: async () => {
      if (editCat) {
        ...
        const { error } = await supabase
          .from('service_categories')
          .insert({ business_id: businessId!, name: catName.trim() });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const openEditCat = (cat: { id: string; name: string }) => {
    setEditCat(cat);
    setCatName(cat.name);
                    ...
                    <div key={cat.id} className="flex items-center justify-between rounded-lg border p-2.5 gap-2">
                      ...
                      {canManage && (
                        <div className="flex items-center gap-0.5 shrink-0">
                          <button className="p-1 rounded hover:bg-muted" onClick={() => openEditCat(cat)}>
