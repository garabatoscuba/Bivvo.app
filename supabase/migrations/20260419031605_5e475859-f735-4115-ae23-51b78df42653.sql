CREATE OR REPLACE FUNCTION public.list_public_storefronts()
 RETURNS TABLE(id uuid, name text, slug text, business_type text, keywords text, logo_url text, hero_image_url text, accent_color text, schedule jsonb, address text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT DISTINCT ON (b.id)
    b.id, b.name, b.slug, b.business_type, b.keywords, b.logo_url,
    ss.hero_image_url, ss.accent_color, ss.schedule, br.address
  ...
  WHERE b.is_active = true
    AND COALESCE(lower(op.email), '') <> 'bivoo.app@gmail.com'
  ORDER BY b.id, br.is_main DESC;
$function$;

CREATE OR REPLACE FUNCTION public.search_public_catalog(q text)
 RETURNS TABLE(kind text, id uuid, name text, price numeric, business_id uuid, business_name text, business_slug text, business_type text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
